...
export async function POST(request) {
  ...
  const userRate = await consumeRateLimit(
    `song-upload:user:${userId}`,
    USER_UPLOAD_LIMIT,
    WINDOW_MS,
  );

  const ipRate = await consumeRateLimit(
    `song-upload:ip:${getClientIp(request)}`,
    IP_UPLOAD_LIMIT,
    WINDOW_MS,
  );
  ...
  try {
      ...
      return NextResponse.json(
        {
          success: false,
          message:
            parsed.error.issues[0]?.message || "Invalid upload details.",
        },
        { status: 400 },
      );
    }
    ...
    if (fileSize > MAX_SONG_SIZE_BYTES) {
      return NextResponse.json(
        { success: false, message: "Maximum song size is 25 MB." },
        { status: 413 },
      );
    }
    ...
    // The temporary placeholder hash is replaced after server-side inspection.
    ...
    if (error || !data?.token) {
      ...
      return NextResponse.json(
        {
          success: false,
          message: "Unable to prepare the upload right now.",
        },
        { status: 503 },
      );
    }
    ...
    return NextResponse.json({
      success: true,
      uploadId: song._id.toString(),
      path: storagePath,
      token: data.token,
      expiresAt: song.uploadExpiresAt,
    });
  } catch (error) {
    console.error("[Suno] Upload initiation error:", error);

    return NextResponse.json(
      { success: false, message: "Unable to start the upload." },
      { status: 500 },
    );
  }
}